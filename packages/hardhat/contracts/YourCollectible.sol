	...
	ReentrancyGuard
{
	...
	event NftExpired(uint256 indexed tokenId, address indexed seller);
	...
	event DefaultRoyaltyPercentageUpdated(uint256 newDefaultRoyaltyPercentage);
	 ...
	 */
	// function mintBatch(address to, string[] memory uris, uint256 quantity) public returns (uint256[] memory) {
	//     require(uris.length == quantity, "URI length must be equal to quantity");
	...
	// // 定时清理过期的NFT
	// function cleanExpiredNfts() external {
	//     for (uint256 i = 0; i < _listedTokenIds.length; i++) {
	//         uint256 tokenId = _listedTokenIds[i];
	//         NftItem storage item = _idToNftItem[tokenId];
	//         if (item.isListed && item.endTime <= block.timestamp) {
	//             // 下架逻辑
	//             _unlistNft(tokenId, false); // 不触发事件
	//         }
	//     }
	// }
	...
	// 查询所有的竞价信息记录
	function getBidRecobirds() external view returns (BidRecord[] memory) {
		return bidRecords;
	}
	...
	function getPurchaseRecord(
		...
		require(index < purchaseRecords.length, "Index out of bounds");
		return purchaseRecords[index];
	}
	...
	/**
	 * @dev 设置新的默认版税比例（仅合约所有者可调用）
	 * @param _newDefaultRoyaltyPercentage 新的默认版税比例（例如250代表2.5%）
	 */
	function setDefaultRoyaltyPercentage(
		uint256 _newDefaultRoyaltyPercentage
	) external onlyOwner {
		require(
			_newDefaultRoyaltyPercentage <= MAX_ROYALTY_PERCENTAGE,
			"Royalty fee cannot exceed 10%"
		);
		defaultRoyaltyPercentage = _newDefaultRoyaltyPercentage;
		emit DefaultRoyaltyPercentageUpdated(_newDefaultRoyaltyPercentage);
	}
